...
export IP_ADDR=$(gcloud compute addresses list --filter="name=$STATIC_IP_NAME" --global --project $PROJECT_ID --format="value(address)")
...
EXISTING_SERVICE_NAME=$(gcloud endpoints services list --filter="serviceName=$DOMAIN_NAME" --format="value(serviceName)")
...
EXISTING_MANAGED_CERT=$(gcloud beta compute ssl-certificates list --filter="name=$MANAGED_CERT" --format="value(name)")

if [ -z "$EXISTING_MANAGED_CERT" ]; then
  bold "Creating managed SSL certificate $MANAGED_CERT for domain $DOMAIN_NAME..."

  gcloud beta compute ssl-certificates create $MANAGED_CERT --domains $DOMAIN_NAME --project $PROJECT_ID
else
...
DECK_SERVICE_TYPE=$(kubectl get service -n spinnaker spin-deck --output=jsonpath={.spec.type})
